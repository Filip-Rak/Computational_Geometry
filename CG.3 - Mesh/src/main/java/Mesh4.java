...
public class Mesh4
{
    ...
    // Constructors
    Mesh4(ArrayList<Point> p_arr, int stride, double snap_range, double snap_range_multiplier, double determination, int ceil, int delay)
    {
        initVars(p_arr, stride, snap_range, snap_range_multiplier, determination, ceil, delay,null);
        buildMesh();
    }

    Mesh4(ArrayList<Point> p_arr, int stride, double snap_range, double snap_range_multiplier, double determination, int ceil, int delay, ArrayList<Polygon> blind_spots)
    {
        initVars(p_arr, stride, snap_range, snap_range_multiplier, determination, ceil, delay, blind_spots);
        buildMesh();
    }

    // Initialization
    private void initVars(ArrayList<Point> p_arr, int stride, double snapRange, double multi, double determination, int ceil, int delay, ArrayList<Polygon> blind_spots)
    {
        // Lists
    ...
    {
        while(points.size() >= 3 && failed_insertions < (points.size() * determination) && iter < ceil)
        {
            iter++;
            System.out.println(
                    "----------------------------------------------------\n" +
                            "Iteration: " + iter + ", Size: " +  points.size() +
